...
def get_top_song_from_playlist(
    ...
    """
    results = sc.SPOTIFY.playlist_tracks(
        playlist_URI, limit=limit, market=market, offset=offset
    )
...
def get_lyrics_from_genius_url(
    ...
    for raw_lyric in raw_lyrics:
        lyric = re.sub(
            re.compile("<.*?>"), sc.SPECIAL_SYMBOL, str(raw_lyric)
        )  # replace all texts inside <???> in '`'
...
def get_lyric_of_top_song(
    ...
    while first_n_song > 0:
        first_n_song -= limit
        top_song_list: List[Song] = get_top_song_from_playlist(
            playlist_URI, offset=len(res), limit=limit, market=market
        )
    ...
    for song in res:
        if song[sc.SONGNAME] in processed_names:
            continue
        try:
            lyric = get_lyrics_from_genius_url(
                generate_genius_url(song), title=title, one_line=one_line
            )
            with_lyrics.append(
                {
                    "song_name": song[sc.SONGNAME],
                    "artist_name": song[sc.ARTIST],
                    "lyrics": lyric,
                }
            )
            ...
            errors += 1
            print(
                f"Error: {e}. {errors} errors so far. Skipping song {song[sc.SONGNAME]} by {song[sc.ARTIST]}"
            )
...
if __name__ == "__main__":
    ...
    res = get_lyric_of_top_song(
        "spotify:playlist:7E3uEa1emOcbZJuB8sFXeK",
        first_n_song=350,
        cache_filename=songs_filename,
    )
